...
from .schema import LicenseVerifiedHttpResponse, DownloadProjectHttpResponse
...
class Client:
    ...
    def verify(self, license_id: str) -> LicenseVerifiedHttpResponse:
        ...
        try:
            res = httpx.post(
                f"{server_url}/projects/authenticate-license/", json={"license_id": license_id}
            )
            data = res.json()
    ...
    def download(self):
        ...
        if data.is_valid is False or data.profile_status == "inactive":
            rich.print("[red]Project Download Failed, the link may have expired!\n Please try again.[/red]")
            return
        ...
        if not response.status_code == 200:
            rich.print("[red]File Download Failed, the link may have expired!\n Please try again.[/red]")
            return
