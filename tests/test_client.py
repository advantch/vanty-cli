...
from vanty.schema import LicenseVerifiedHttpResponse
...
def test_download_zip(client, respx_mock):
    respx_mock.get("https://example.com/test.zip").mock(
        return_value=httpx.Response(200, content=b"Test zip content")
    )
...
def test_verify_success(client, mock_config, respx_mock):
    ...
    respx_mock.post(
        "https://www.advantch.com/api/v1/projects/authenticate-license/"
    ).mock(
...
def test_verify_failure(client, mock_config, respx_mock):
    ...
    respx_mock.post(
        "https://www.advantch.com/api/v1/projects/authenticate-license/"
    ).mock(return_value=httpx.Response(400, json={"error": "Invalid token"}))
